...
def drawBoard(screen, board):
    ...
    for i in range(board.boardSize):
        for j in range(board.boardSize):
            ...
            if board.board[i][j] != 0:
                gComponent = 235 - \
                    log2(board.board[i][j])*((235 - 52)/(board.boardSize**2))
    ...
    fontImage = scoreFont.render("Score: {:,}".format(
        board.score) + (" [AI enabled, depth={}]".format(depth) if ai else ""), 1, white)
    ...
    for depth in range(1, 10):
        for game in range(10):
            ...
            while True:
                ...
                if not board.checkLoss():
                    nextBestMove = getNextBestMoveExpectiminimax(
                        board, pool, depth)
                    board.move(nextBestMove)
                    moves += 1
                else:
                    print(board.score)
                    games.append([depth, board.score])
                    os.system('cls')
                    print(games)
                    sleep(1)
                    break
                if moves % 100 == 0:
                    print('Move', moves, 'Score', board.score)
